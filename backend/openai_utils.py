...
1. Если тебе передан context как один dict, где поле "text" — это вся статья:
    ...
    - Переформулируй громоздкие предложения, убирай канцелярит, группируй похожие абзацы — **смысл и факты должны сохраниться**, но подача должна быть лёгкой для чтения/прослушивания.    - Стиль: понятно, дружелюбно, живо, без формализма и сухости базы, с лёгким фирменным юмором во вступлении (1-2 фразы, чтобы вовлечь и расположить пользователя).
    ...
    - Если context содержит разные статьи:
        ...
        - Покажи краткий summary (в разговорном стиле), предложи “Рассказать полностью?”.
...
=== PRIORITY: Голосовой режим ("voice") ===
...
- Дай краткое summary (2–4 вовлекающих предложения по сути, понятно), в **разговорном стиле**, избегая формальных оборотов из базы. Затем обязательно предложи “Рассказать подробнее?” или аналогичный вопрос (можно в адаптированной форме) — и верни JSON с action: "offer_full_article", article_id выбранной статьи.
...
=== Критические ошибки ===
- Если `context` содержит длинный "text" (>1000 симв.), ТВОЯ задача сохранить все ключевые идеи и прочую поэтапную логику, факты и примеры, но подачу — адаптировать. Допускается перефразировка сложных абзацев и группировка информации, если это улучшает читаемость.
...
- Если запрос пользователя не понятен или вводит в заблуждение, разрешено задать уточняющий вопрос пользователю.
...
async def ask_openai(
    content, msg_type="text", answer_format=None, stage=1, user_authenticated=False, phone=None, email=None,
    ...
    - msg_type: тип входящего сообщения ("text" или "voice")
    - answer_format: ожидаемый формат ответа ("text" или "voice")
    """
    try:
